...
（最終更新: 2026-07-24。スナップショット数値は更新時点のもの）
...
**現在地（2026-07-24 時点）**:
- 主要な分析は一巡し**結論が出ている**（§8）。フォワード検証は30日読み（08-02）まで完了し
  「スキルの前向き持続」は棄却・**「現在スキル選抜」路線は廃止**（§8.6）。直近成果物は
  `調査まとめ_2026-07-24.docx`（v5、フォワード検証反映。`make_report.py` で生成、
  **git 未追跡**・snitker NAS `document` にコピー済み。v4=07-09 版も NAS にある）。
...
| `make_report.py` | Word レポート生成（現行 v5 = `調査まとめ_2026-07-24.docx`）。**要 python-docx（venv）** |
...
6. **スキルは前向きには持続しなかった — 30日読み（08-02）で確定・路線廃止**:
   07-02 検証済み40人、OOS 31.2日: プラスは 12/25（48% = 正確にコインフリップ、p=1.00。
   in-sample の「85% プラス維持」は p≈1.7e-5 で棄却）。合計 OOS net −$410K、
   **span≥1d（境界アーティファクト除去後）では 8/20 プラス・−$920K** — BTC が +4.9%
   上昇する中で。in-sample PF の予測力は ρ=+0.04（初回 −0.20 → ゼロ周りのノイズと確定）。
   OOS 単独で再検証に通るのは 4/40（10%、n=4 は逸話）。離脱 14/40。ファンネル生存重複は
   stale-window アーティファクトで測定値として死亡。skill_findings の split-half 持続は
   「同一レジーム共有」の caveat が本質だったと確定。**「現在スキル選抜」は追わない。**
   詳細 `notes/forward_findings.md` Addendum。
...
| 完了（07-24） | Word レポートは **v5**（`調査まとめ_2026-07-24.docx`、フォワード検証初回読みを筆頭知見に reframe）まで更新し NAS `document` へコピー済み。v4（07-09、対照群反映）も NAS にある。次の改版は 8月頭の3検証（§11 下記）が出揃ってから |
...
| 完了（08-02） | **フォワード検証の30日読み**: 初回読みの結論が全て維持され、**「現在スキル選抜」路線は廃止と決定**（48% プラス=コインフリップ、in-sample PF の予測力 ρ=+0.04、span≥1d に絞ると −$920K vs BTC +4.9%。初回読みの「BTC +2.0%」は起点価格の誤りで実際は +4.9% — 結果はより悪い）。詳細 `notes/forward_findings.md` Addendum |
