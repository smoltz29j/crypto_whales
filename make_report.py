...
sr = sub.add_run("2026年7月5日（7/6 母集団拡張版・50人/4,564トリップに更新） / crypto_whales プロジェクト")
sr.font.size = Pt(10)
...
doc.add_heading("2.2 上手い人の実際の手法（BTC・4,564トリップ / 50人の分析）【確度: 高】", level=2)
p("検証済みスキル保有者のうち、建玉をフラットに戻す「方向性トレーダー」全員の"
  "個別トレード（トリップ）を全取引履歴から抽出し、セットアップを分析した結果"
  "（7/6 に母集団を初回の17人/574トリップから50人/4,564トリップへ8倍に拡張し、"
  "全判定を再検定済み）：")
...
table(
    ...
    [
        ["① 損切りは速く、勝ちは伸ばす",
         "負けトレードの保有時間は勝ちの0.39倍（中央値）。全体では負け51分 vs 勝ち"
         "2.1時間（高速勢は数分で投げる。普遍なのは分数ではなく比率）。勝率は48%しか"
         "ないのに非対称なペイオフで儲けている。トップ勢は勝率20〜40%でペイオフ比20倍超。",
         "確立（母集団拡張で更に強化）。41/46人（符号検定 p=2.2×10⁻⁸、"
         "合成 p≈10⁻³⁵、95%CI [0.29, 0.48]、スクラッチ除外でも頑健）"],
        ["② 追いかけず、逆張りする",
         "直前4時間の値動きと同方向のエントリーは39%のみ。ロングは中央値−0.42%の"
         "下落後に入る。これは「彼らがそう行動する」という事実。ただし『逆張りの方が"
         "順張りより勝ちやすい』というエッジの証拠は、トレーダー層別の検定では出ない。",
         "行動としては確か。エッジとしては不成立"
         "（CMH OR 1.05, p=0.56）"],
        ["③ ボラティリティが引き金",
         "エントリーの40%は4時間で1%以上動いた直後（ただし勝率は静穏時と同じ51%）。",
         "不成立と決着。初回（n=574）の p=0.074 は8倍の母集団で消滅"
         "（OR 1.02, p=0.78）— ノイズだった"],
        ["④ 積み増し（ピラミッディング）は常態",
         "74%のトリップでポジションに追加（中央値6回）。少額から100万ドル超の建玉を"
         "組み立てる例も。『追加した方が勝ちやすい』かは母集団によって結果が揺れる"
         "（初回 p=0.91 → 拡張後 p=0.076）ため、効果なしとして扱う。",
         "行動としては確か。エッジとしては不安定・不成立"
         "（CMH OR 1.14, p=0.076）"],
        ["⑤ 時間帯はNYセッション",
...
p("統計処理の要点：プール集計は高頻度口座に引きずられるため、勝率の比較はすべて"
  "トレーダー層別の Cochran-Mantel-Haenszel 検定で行いました。初回（17人）で"
  "「発見」に見えた②④のエッジは合成の錯覚（Simpson のパラドックス）、③の示唆も"
  "母集団を8倍にしたら消えました。統計的に生き残ったのは①の撤退規律だけで、"
  "こちらは拡張でむしろ桁違いに強くなっています（p≈10⁻⁹ → 10⁻³⁵）。", bold=False)
...
numbered("統計的に確立した発見は一つだけで、それは「エントリーの上手さ」ではなく"
         "「エグジット規律」です。勝率48%でも、負けに与える時間を勝ちの4割以下に"
         "絞れば勝てる — 50人中41人に共通し（合成 p≈10⁻³⁵）、タイムフレームを問わず"
         "比率ルールとして個人のトレードに移植できる唯一の再現性ある要素です。",
         bold_prefix="エグジット規律がすべて。")
numbered("上手い人が追いかけないのは事実ですが、「逆張りの方が勝てる」「大変動後の方が"
         "勝てる」「積み増しは勝ちやすい」といったエントリー側の仮説は、8倍に拡張した"
         "母集団の層別検定で全て不成立でした。エントリー手法の真似より、出口の規律の"
         "移植に集中するのが正しい読み方です。",
...
numbered("§2.2 の手法分析は「勝者の習性」であって因果ではありません。負けトレーダー群に"
         "同じトリップ分析をかけ、「速い損切り」が本当に勝者と敗者を分けるのかを"
...
bullet("トリップは同一トレーダー内で独立ではない（時間的クラスタ・同一相場）ため、"
       "p値はやや楽観的です。クロストレーダーの主張の実効サンプルは50人"
       "（初回17人から拡張済み）。")
bullet("スキルの検証は直近2000件の約定で行い、行動分析は全取引履歴"
       "（1人あたり最大12,000件までページング取得）で行っています。古いトリップは"
       "相場環境が現在と異なる点に注意。")
